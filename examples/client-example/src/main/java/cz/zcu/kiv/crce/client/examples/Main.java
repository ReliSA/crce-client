...
public class Main {
	...
	public static void coreLibrarySmokeTest1() throws IOException, InterruptedException {
		...
		final String FILE_NAME = "downloaded.jar";
		final OutputStream STREAM = new FileOutputStream(LOCAL_DIR + File.separator + FILE_NAME);
		...
		cl.setServerURI(RELISA_URI);
		cl.getResourceBinary(id, STREAM);
		File f = new File(cl.getBinaryFilePath() + File.separator + FILE_NAME);
		myAssert(f.exists());
